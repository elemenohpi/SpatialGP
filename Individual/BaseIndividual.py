...
class BaseIndividual(AbstractIndividual):
    ...
    def init_random(self):
        ...
        for i in range(program_count):
            ...
            if (
                not self.has_discrete_output
                and not self.output_ratio == "single"
                and not self.output_ratio == "none"
            ):
    ...
    def get_annotation(self):
        ...
        for index, program in enumerate(self.programs):
            ...
            cost_pos_txt = self.generate_method_attribute_text(
                cost, pos, return_var, program_type, discrete_output
            )
            full_program = (
                class_txt
                + self.add_indent(cost_pos_txt)
                + "\n\n"
    ...
    def generate_method_attribute_text(
        self, cost: int, pos: tuple, return_var, ptype, discrete_output
    ):
        if return_var.__class__.__name__ == "Retcon":
            return_var = return_var.annotate()
            cost_pos_txt = (
                'discrete_output = \'{}\'\ntype = \'{}\'\nreturn_var = """{}"""\ncomplexity = {}\n'
                "pos = {}\nvisit_count = 0".format(
                    discrete_output, ptype, return_var, cost, str(pos)
                )
            )
        else:
            cost_pos_txt = (
    ...
    def evaluate(self, problem_inputs):
        ...
        while True:
            ...
            if self.analysis:
                ...
                self.analysis_executed_statements_count += len(
                    current_program.statements
                )
    ...
    def select_program(self, current_program, programs):
        ...
        if selected_program is not None:
            selected_program.visit_count += 1
            selected_program.logged_cost = abs(
                selected_program.cost(source_pos, self.internal_state)
            )
    ...
    def initialize_memory(self, problem_inputs):
        ...
        if not self.passes_input_sanity(problem_inputs):
            raise ValueError(
                "The provided input list does not match the scheme specified in the problem/fitness file"
            )
        ...
        for input_element in input_scheme:
            if self.is_array(input_element):
                for index in range(self.get_array_length(input_element)):
                    ...
                    self.internal_state[name + str(index)] = problem_inputs[
                        unique_variable_counter
                    ][index]
            else:
                self.internal_state[input_element] = problem_inputs[
                    unique_variable_counter
                ]
    ...
    def passes_input_sanity(self, problem_inputs):
        ...
        for index, input_element in enumerate(input_scheme.keys()):
            ...
            if self.is_array(input_element) and type(problem_inputs[index]) != list:
                ...
                self.is_array(input_element)
                and type(problem_inputs[index]) == list
                and len(problem_inputs[index]) != self.get_array_length(input_element)
            ):
                return False
            elif (
                not self.is_array(input_element) and type(problem_inputs[index]) == list
            ):
                return False
        return True
