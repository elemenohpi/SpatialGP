...
class I918(AbstractFitness):
    ...
    def evaluate(self, individual):
        ...
        for i in range(self.evaluation_count):
            # ======================================STARTPROBLEM===============================================
            x = self.data_handler.get_data(2)
            y = self.data_handler.get_data(2)
            z = self.data_handler.get_data(2)
            m = self.data_handler.get_data(2)
            G = self.data_handler.get_data(1)

            inputs = [x, y, z, m, G]

            measured = G * m[0] * m[1] / ((x[1] - x[0]) ** 2 + (y[1] - y[0]) ** 2 + (z[1] - z[0]) ** 2)
