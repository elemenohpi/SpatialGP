...
class II615a(AbstractFitness):
    ...
    def evaluate(self, individual):
        ...
        for i in range(self.evaluation_count):
            # ======================================STARTPROBLEM===============================================
            pd = self.data_handler.get_data(1)
            pi = math.pi
            r = self.data_handler.get_data(1)
            z = self.data_handler.get_data(1)
            x = self.data_handler.get_data(1)
            y = self.data_handler.get_data(1)
            e = self.data_handler.get_data(1)

            inputs = [pd, pi, r, z, x, y, e]

            measured = 3 / (4 * pi * e) * pd * z / r ** 5 * math.sqrt(x ** 2 + y ** 2)
            # ======================================ENDPROBLEM===============================================
