...
def circle_crossover(a, b, config):
    ...
    max_program_count = int(config["size_max"])
    ...
    if len(offspring_a.programs) == 0 or len(offspring_a.programs) > max_program_count:
        offspring_a = copy.deepcopy(a)
    elif len(offspring_b.programs) == 0 or len(offspring_b.programs) > max_program_count:
